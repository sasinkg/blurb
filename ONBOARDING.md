...
September 18, 2026 authentication update: the welcome screen now offers Continue with email for existing Firebase email/password accounts, alongside Sign in with Apple. EmailAuthView.swift provides sign-in and password reset; there is no public email sign-up form. AuthManager waits for profile setup before publishing a new session and surfaces profile-save failures separately from sign-in failures. Account-deletion reauthentication instructions now apply to either provider. Enable Email/Password in Firebase and create a dedicated review user in the console, then populate sample content and supply credentials privately to App Store Connect. See README.md, App Review account, for setup and review notes. No review credentials are embedded in source and no cloud account was created by this change.

Validation for that update: Debug simulator build succeeded with Xcode 27 using its internal-disk SourcePackages cache and CODE_SIGNING_ALLOWED=NO. The T7 dependency cache failed because AppleDouble ._* header files were picked up by the compiler. git diff --check passed. A UI smoke test covers empty-field submission, password reset availability, cancellation, and field clearing, but was not run because no simulator devices were available; the user will run screen testing. Real Firebase sign-in/reset and Apple sign-in still require manual verification. No cloud provider settings, account, deployment, or uploaded build were changed.
...
September 18 follow-up: email sign-up is now available as requested. EmailAuthView uses separate labeled inputs, a blue primary button, a gray Sign up/Back to sign in button, and a small Forgot password link. SignedInRootView waits for the user profile and routes incomplete accounts through ProfileSetupView (required display name, optional photo) before ContentView/group onboarding. New profiles have profileSetupCompleted=false; saving a profile sets it true. Legacy profiles without the marker are complete only when their name is nonempty and not Blurb friend. This supersedes the earlier note that there is no public email registration. The previously created review account may receive setup if its profile still has the default name.

September 18 review examples/version update: version 1.1/build 3 is configured. ReviewConfiguration.swift contains the user-provided non-secret Firebase review UID. Only that account sees Add example group during group onboarding or Settings → Review examples, and BlurbStore checks the UID before seeding. The action creates/reuses an owned private Example group, adds three fictional sample answers and one reply per answer, and preserves existing sample documents on retries. Group, posts, and comments commit in separate stages to satisfy current rules. Sample records use the review account's authorID, fictional labeled names, zero points, and no streak credit. Profile renames don't rewrite fictional sample names. Example-group feeds show these sample posts without the daily-answer UI gate; ordinary groups are unchanged. Photos and archived newsletter editions are not seeded. Existing newsletter previews can include the reflection samples; the scheduler may later archive them. Nothing was written to live Firebase during implementation; the user must run the action in the updated app. No backend rules were loosened and no fake Auth accounts were created. If the review Auth user is deleted/recreated, update its configured UID. Simultaneous creation from multiple devices can create more than one example group; ordinary repeat taps reuse an owned group.

Validation: Release simulator build succeeded for the sample-group update. Built Info.plist reports sasinkg.blurb, version 1.1, build 3. git diff --check passed. Live seeding, retry preservation, and interactions still need device verification by the user; no new archive/upload/release was performed.


Answer-lock correction: the example group now uses the same per-group daily-answer UI gate as ordinary groups. Sample posts never count as the current user's answer. Deleting the qualifying answer or changing the prompt closes open feed comment/edit sheets and relocks the feed. Current newsletter previews filter out responses to prompts the user has not answered (sample entries use today's gate), preventing that alternate in-app route from revealing locked answers. Archived editions retain their existing behavior. This is still an application presentation rule; Firestore rules continue to authorize reads by group membership rather than submission state.


Review-account prompt/count update: only the configured review UID receives the fixed question “What would you like to share with the group today?” on Home, group feeds, and the current newsletter gate. Its prompt ID changes at 5 AM Pacific each day, so answering still unlocks only that day's conversation. Sample captions and current newsletter preview questions use the same wording, including for previously seeded records (no cloud rewrite required). Other users keep QuestionBank prompts. Example cards display three fictional participants plus actual group members, with the three samples included in the labeled display count (3/4 before the review user's answer, 4/4 afterward); real membership and answer-gate logic are unchanged. Older saved newsletter editions and stored sample prompt fields are not rewritten.
